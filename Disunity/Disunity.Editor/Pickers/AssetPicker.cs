...
namespace Disunity.Editor {
    public class AssetPicker : EditorWindow {
        ...
        public static void ObjectField(
            ...
            string controlName = "ObjField" + label;
            var e = Event.current;
            bool showHotKey =
                GUI.GetNameOfFocusedControl() == controlName &&
                e.type == EventType.KeyDown &&
            ...
            var guiContent = EditorGUIUtility.ObjectContent(currentValue, assetType);
            ...
            style.imagePosition = currentValue ? ImagePosition.ImageLeft : ImagePosition.TextOnly;


            if (GUILayout.Button(guiContent, style) && currentValue)
                EditorGUIUtility.PingObject(currentValue);

            if (deleteHotKey) {
                pickCallback(null);
            } else if (showHotKey || GUILayout.Button("wtf")) {
                // invisible button overrides object picker
                Show(
                    assetType,
                    null,
        ...
        public IEnumerable<Type> CollectAssetTypes() {
            ...
            var soTypes = TypeExtensions.GetSubclasses<ScriptableObject>(true, gameAssembly);
